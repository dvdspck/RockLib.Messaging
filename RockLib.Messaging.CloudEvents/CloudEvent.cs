...
namespace RockLib.Messaging.CloudEvents
{
    ...
    public class CloudEvent
    {
        ...
        private static readonly string[] _specVersions = new[]
        {
            "1.0",
            "1.0.1",
            "1.0.2"
        ...
        /// </summary>
        public static string SpecVersion => _specVersions[0];
...
#pragma warning restore // Properties should not return arrays
        ...
        private void FromJson(string json)
        {
            ...
            foreach (var attribute in jobject)
            {
                ...
                if (attribute.Key == "specversion" && attribute.Value is JValue jv && jv.Value?.ToString() is string specVersion)
                {
                    ...
                    throw new CloudEventValidationException(
                        $"Invalid '{SpecVersionAttribute}' attribute. Expected one of '{( string.Join(", ", _specVersions) )}', but was '{specVersion}'.");
                }
        ...
        private static bool IsSpecVersionSupported(string specVersion) =>
            _specVersions.Contains(specVersion);
        ...
        public static void Validate(SenderMessage senderMessage, IProtocolBinding? protocolBinding = null)
        {
            ...
            {
                throw new CloudEventValidationException($"The '{specVersionHeader}' header must have a value of one of ''{( string.Join(", ", _specVersions) )}''.");
            }
